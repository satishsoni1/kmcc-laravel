...
@section('content')
@include('partials._page-header', ['title' => 'Syllabus', 'breadcrumbs' => ['Academics' => route('academics.index'), 'Syllabus' => null]])
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <main class="lg:col-span-2 space-y-6">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($years as $y)
                <a href="{{ route('academics.syllabus', ['year' => $y]) }}"
            ...
            <div class="bg-white rounded-xl shadow-md p-8">
                <h2 class="text-2xl font-bold text-blue-900 mb-2">Syllabus — {{ $year }}</h2>
                <div class="w-12 h-1 bg-yellow-500 mb-5"></div>

                @forelse($syllabi as $programme => $docs)
                <div class="mb-6">
                    <h3 class="font-bold text-blue-900 bg-blue-50 px-4 py-2 rounded-lg mb-2 text-sm uppercase tracking-wide">
                        {{ $programme ?: 'General' }}
                    </h3>
                    <div class="space-y-2">
                        @foreach($docs as $doc)
                        <div class="flex items-center justify-between border border-gray-200 rounded-lg px-4 py-3 hover:bg-blue-50 transition-colors">
                            <div>
                                ...
                                @if($doc->department)<p class="text-xs text-gray-500">{{ $doc->department }}</p>@endif
                            </div>
                            @if($doc->file_path)
                ...
                <div class="text-center text-gray-400 py-12">
                    ...
                    <p>Syllabus for {{ $year ?? 'this year' }} will be available soon.</p>
                </div>
                @endforelse
            </div>
        </main>
