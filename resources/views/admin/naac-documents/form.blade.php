...
@section('title', isset($document) ? 'Edit NAAC Document' : 'Add NAAC Document')
@section('page-title', isset($document) ? 'Edit NAAC Document' : 'Add NAAC Document')

@section('content')
<div class="max-w-2xl">
    <form action="{{ isset($document) ? route('admin.naac-documents.update', $document) : route('admin.naac-documents.store') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <select name="type" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @foreach($types as $t)
                    <option value="{{ $t }}" {{ old('type', $document->type ?? '') == $t ? 'selected' : '' }}>{{ \App\Models\NaacDocument::typeLabel($t) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="cycle" value="{{ old('cycle', $document->cycle ?? '') }}"
                    placeholder="e.g. 3rd" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div>
                ...
                <input type="text" name="academic_year" value="{{ old('academic_year', $document->academic_year ?? '') }}"
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Grade (for Grading type)</label>
                <input type="text" name="grade" value="{{ old('grade', $document->grade ?? '') }}"
                    placeholder="e.g. A++" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="{{ route('admin.naac-documents.index') }}" class="px-5 py-2 rounded-lg bg-gray-100 text-gray-700 text-sm font-medium hover:bg-gray-200">Cancel</a>
        </div>
    </form>
</div>
@endsection
