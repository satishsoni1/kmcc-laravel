...
@section('title', isset($document) ? 'Edit Exam Document' : 'Add Exam Document')
@section('page-title', isset($document) ? 'Edit Exam Document' : 'Add Exam Document')

@section('content')
<div class="max-w-2xl">
    <form action="{{ isset($document) ? route('admin.exam-documents.update', $document) : route('admin.exam-documents.store') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <select name="type" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @foreach($types as $t)
                    <option value="{{ $t }}" {{ old('type', $document->type ?? '') == $t ? 'selected' : '' }}>{{ \App\Models\ExamDocument::typeLabel($t) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="semester" value="{{ old('semester', $document->semester ?? '') }}"
                    placeholder="e.g. Sem I, Sem II" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">Upload File (optional if external link provided)</label>
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">External Link (for results hosted online)</label>
                <input type="url" name="external_link" value="{{ old('external_link', $document->external_link ?? '') }}"
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="{{ route('admin.exam-documents.index') }}" class="px-5 py-2 rounded-lg bg-gray-100 text-gray-700 text-sm font-medium hover:bg-gray-200">Cancel</a>
        </div>
    </form>
</div>
@endsection
