...
@section('page-title', 'IQAC Documents — AQAR, SSS, Calendar, Policies, Minutes')
...
<form method="GET" class="flex flex-wrap items-center gap-3 mb-5">
    <select name="type" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        @foreach($types as $t)
        <option value="{{ $t }}" {{ request('type') == $t ? 'selected' : '' }}>{{ \App\Models\IqacDocument::typeLabel($t) }}</option>
        @endforeach
    </select>
    ...
    <a href="{{ route('admin.iqac-documents.index') }}" class="px-4 py-2 rounded-lg bg-gray-100 text-gray-700 text-sm hover:bg-gray-200 transition-colors">Clear</a>
    <a href="{{ route('admin.iqac-documents.create') }}" class="ml-auto flex items-center gap-2 px-4 py-2 rounded-lg text-white text-sm font-semibold hover:opacity-90" style="background-color:#2d4077;">
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                <td class="px-4 py-3">
                    ...
                    @if($doc->description)<p class="text-xs text-gray-400 mt-0.5">{{ Str::limit($doc->description,60) }}</p>@endif
                </td>
                <td class="px-4 py-3">
                    <span class="text-xs px-2 py-1 rounded-full bg-indigo-50 text-indigo-700 font-medium">{{ \App\Models\IqacDocument::typeLabel($doc->type) }}</span>
                </td>
                ...
                <td class="px-4 py-3 text-right">
                    <div class="flex items-center justify-end gap-2">
                        ...
                        <a href="{{ route('admin.iqac-documents.edit', $doc) }}" class="text-xs px-3 py-1.5 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700">Edit</a>
                        <form action="{{ route('admin.iqac-documents.destroy', $doc) }}" method="POST" onsubmit="return confirm('Delete?')">
                            @csrf @method('DELETE')
            ...
            @empty
            <tr><td colspan="5" class="px-4 py-12 text-center text-gray-400">No IQAC documents found.</td></tr>
            @endforelse
        </tbody>
    </table>
