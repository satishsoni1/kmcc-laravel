...
@section('title', isset($document) ? 'Edit IQAC Document' : 'Upload IQAC Document')
@section('page-title', isset($document) ? 'Edit IQAC Document' : 'Upload IQAC Document')

@section('content')
<div class="max-w-2xl">
    <form action="{{ isset($document) ? route('admin.iqac-documents.update', $document) : route('admin.iqac-documents.store') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <select name="type" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @foreach($types as $t)
                    <option value="{{ $t }}" {{ old('type', $document->type ?? '') == $t ? 'selected' : '' }}>{{ \App\Models\IqacDocument::typeLabel($t) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                @endif
                <input type="file" name="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.png" {{ isset($document) ? '' : 'required' }}
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="{{ route('admin.iqac-documents.index') }}" class="px-5 py-2 rounded-lg bg-gray-100 text-gray-700 text-sm font-medium hover:bg-gray-200">Cancel</a>
        </div>
    </form>
</div>
@endsection
